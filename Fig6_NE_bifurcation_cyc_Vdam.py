...
"""model HAT
Goal : Final value os CycE as function of Vdam (bacward and forward)
"""
...
def make_stopping_event(systeme_diff, param_init, M1, M2, M3, rep, eps=1e-6):
    def event_func(t, y):
        if y[25] >100:  # seuil à ajuster selon ton système
            return 1.0  # => Pas encore de stabilisation, ne déclenche pas
        dydt = systeme_diff(y, t, param_init, M1, M2, M3, rep)
        return abs(dydt[25]) - eps
...
# Linspace entre 3 et 10 avec 100 valeurs
vect_Vdam = np.linspace(1e-5,1.5e-4,40)

tmin=0 #time min
tmax=40*24*3600#20*24*3600 #time max in days (20 days)
nbptstime=100000 #nb points for time vector
t = np.linspace(tmin, tmax, nbptstime) 
#Species initialization for the edo resolution
...
#vector of CycE final values (initialized empty)
vect_fin_cyc=[]
verif=[]
for i in range(len(vect_Vdam)):
    ...
    event_func = make_stopping_event(systeme_diff, param_init, M1, M2, M3, rep)
    sol= solve_ivp(lambda t, y: systeme_diff(y, t, param_init,M1,M2,M3,rep), 
                [tmin, tmax], 
                species_init,method='BDF', 
                t_eval=t,atol=1e-6, rtol=1e-6,events=event_func)
    ycyc=sol.y[25] #CycE solution
    verif.append(len(ycyc))
    vect_fin_cyc.append(ycyc[-1]) #final value of CycE
    species_init= sol.y[:, -1] #For the continuation the edo has to be solved with the lattest values as initialization

########################## Vdam backward #############################
vect_Vdam_back=np.linspace(1.5e-4,1e-5,40) #e-5 e-3 ok
...
for i in range(len(vect_Vdam_back)):
    Vdam=vect_Vdam_back[i]
    ...
    vect_fin_cyc_back.append(ycyc_back[-1])
    species_init= sol.y[:, -1] #For the continuation the edo has to be solved with the lattest values as initialization
...
cyctresh=100
(max,ind_cyc)=find_max_grad(vect_fin_cyc,vect_Vdam) 
indcross= np.where(np.diff(np.sign( np.array(vect_fin_cyc)-cyctresh)))[0]
vmax=1e-3
ind_cyc=indcross[0]
Vdame=vect_Vdam[ind_cyc]
Vdame_r=round(Vdame,6)
plt.figure(0)
plt.plot(vect_Vdam,vect_fin_cyc,label="Vdam forward",color="green")
plt.plot(vect_Vdam_back,vect_fin_cyc_back,label="Vdam backward",color="blue")
plt.xticks(rotation=45)
plt.text(Vdame+0.00003,10000,fr"${{V_{{dam}}^{{*}}}}\simeq {Vdame_r}$",color='green')
plt.xlabel("Vdam (mmol/ml/s)")
plt.ylabel("CycE final value (mmol/ml)")
plt.legend()
plt.tight_layout()
plt.savefig('Fig6_NE_Vdam_bifurcation_Cyc.png', dpi=dpi)
plt.show()
