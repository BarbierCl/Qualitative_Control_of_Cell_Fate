...
def systeme_diff(y,t,param,M1,M2,M3,rep,pmax,pb,sigma,m):
    ...
    #dx16dt=(h1*DNA_DSB_due_to_IR*is_IR_switched_on+h2*x39)*(DNA_DSB_max-x16)-rep*x16+(pmax-pb)*np.exp(-(t-m)**2/sigma**2)+pb
...
#vecteur des espèces (variables) avec conditions initiales
...
vect_sigma=np.linspace(50000,100000,100)
...
#find bifurcation index
ind_casp=np.where(np.array(vect_fin_casp)>96900)[0]
learning_time=vect_delta[ind_casp[0]]
plt.figure(0)
plt.plot(vect_delta,vect_fin_casp,"--x")
plt.text(2.8,60000,fr"$p_0=${p0}")
plt.text(2.8, 50000, fr"$p_{{max}}=${pmax}")
plt.text(2.6, 0, fr"$\tau=${round(learning_time,2)}")
plt.xlabel(r"${\delta}_{p_{0}}(V_{dam})$ (days)")
plt.ylabel(r"$R_{p_0}(Casp^*)$ (mmol/ml)")
plt.tight_layout()
plt.savefig('Fig7_NE_Vdam_learning_duration_Casp.png', dpi=dpi)
plt.show()

#plotting of Vdam signal
tmin=0 #time min
m=2*24*3600
nbptstime=1000
p0=4e-5#•1.65e-5
...
tmax=m+delta/2
t = np.linspace(tmin, tmax, nbptstime)
plot_Vdam=(pmax-pb)/(1-np.exp(-m**2/sigma**2))*(np.exp(-(t-m)**2/sigma**2)-1)+pmax
indcross= np.where(np.diff(np.sign( plot_Vdam-p0)))[0]
plt.figure(2)
plt.plot(t/(24*3600),plot_Vdam)
plt.xlabel("time (days)")
plt.ylabel("$V_{dam}$ (mmol/ml/s)")
plt.plot([0,2],[pmax,pmax],linestyle='--',color='red')
plt.plot([0,t[indcross[1]]/86400],[p0,p0],linestyle='--',color='red')
plt.plot([2.25,2.25],[0,p0],linestyle='--',color='red')
plt.text(-0.7, pmax, fr"$p_{{max}} = {pmax}$", color='red')
plt.text(-0.6,p0,fr"$p_{0}={p0}$",color='red')
plt.text(2.25,-0.00005,"$t_1$",color="red")
plt.text(1.85,p0+0.00002,"$\delta_{p_0}(V_{dam})$",color='green')
plt.plot([t[indcross[0]]/86400,t[indcross[0]]/86400+delta/86400],[p0,p0],color='green')
plt.xticks([0,1,2])             # Positions des ticks sur l'axe x
plt.yticks([0,0.0002]) 
plt.tight_layout()
plt.savefig('Fig7_NE_bump_Vdam.png', dpi=dpi)
plt.show()
