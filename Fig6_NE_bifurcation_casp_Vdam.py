...
########################## Vdam forward #############################
#vect_Vdam=np.linspace(1e-4,3e-4,40) #Vdam values
vect_Vdam=np.linspace(1.4e-4,2e-4,40) #Vdam values
#time
tmin=0 #time min
tmax=20*24*3600 #time max in days (20 days)
nbptstime=100000 #nb points for time vector
t = np.linspace(tmin, tmax, nbptstime) 
#
apop_tresh=96900 #apoptosis treshold
...
vect_fin_casp=[] #final value of caspase vector

for i in range(len(vect_Vdam)): #variation of the parameter Vdam
    Vdam=vect_Vdam[i]
    ...
    ycasp=sol.y[38] #Casp solution
    vect_fin_casp.append(ycasp[-1]) #final value of Casp
    species_init= sol.y[:, -1] #For the continuation the edo has to be solved with the lattest values as initialization

########################## Vdam backward #############################
#vect_Vdam_back=np.linspace(3e-4,1e-4,40) 
vect_Vdam_back=np.linspace(2e-4,1.4e-4,40) 
species_init=(x1i, x2i, x3i, x4i,x5i, x6i, x7i,x8i,x9i,x10i,x11i,x12i,x13i,x14i,x15i,x16i,x17i,x18i,x19i,x20i,x21i,x22i,
          x23i,x24i,x25i,x26i,x27i,x28i,x29i,x30i,x31i,x32i,x33i,x34i,x35i,x36i,x37i,x38i,x39i,x40i,x41i,x42i)
...
for i in range(len(vect_Vdam)):
    ...
    sol= solve_ivp(lambda t, y: systeme_diff(y, t, param_init,M1,M2,M3,rep), 
                ...
                t_eval=t,atol=1e-6, rtol=1e-6)
    ycasp_back=sol.y[38]
    vect_fin_casp_back.append(ycasp[-1])
    species_init= sol.y[:, -1]


##### plotting 
dpi=300 #dpi resoultion for graphics
#find the bifurcation p*
ind=np.where(np.array(vect_fin_casp)>apop_tresh) #find the first index where vect_fin_casp is greater than apop_tresh
pe=vect_Vdam[ind[0][0]] #p* is the Vdam value corresponding to index ind
round_pe=round(pe,7)

plt.figure(0)
plt.plot(vect_Vdam,vect_fin_casp,label="Vdam forward",color="green")
plt.plot(vect_Vdam_back,vect_fin_casp_back,label="Vdam backward",color="blue")
plt.plot([round_pe,round_pe],[0,max(vect_fin_casp)],color='green', linestyle='dashed')
plt.text(round_pe+0.000005,0,fr"${{V_{{dam}}^{{*}}}}\simeq {round_pe}$",color='green')
plt.xticks(rotation=45)
plt.xlabel("Vdam (mmom/ml/s)")
plt.ylabel("$Casp$ final value (mmol/ml)")
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('Fig6_NE_Vdam_bifurcation_Casp.png', dpi=dpi)
plt.show()
